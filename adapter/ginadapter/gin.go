...
func (a *GinAdapter) Handle(method, path string, h http.Handler, mws ...func(http.Handler) http.Handler) {
	a.register(method, path, h.ServeHTTP, mws...)
}

// HandleFunc registers a new route with a http.HandlerFunc and optional middlewares.
func (a *GinAdapter) HandleFunc(method, path string, h http.HandlerFunc, mws ...func(http.Handler) http.Handler) {
	a.register(method, path, h, mws...)
}

// ANY registers the same handler for GET, POST, PUT, DELETE, PATCH and OPTIONS.
func (a *GinAdapter) ANY(path string, h http.HandlerFunc, mws ...func(http.Handler) http.Handler) {
	methods := []string{
		http.MethodGet,
		http.MethodPost,
		http.MethodPut,
